...
def query_voucher_order_status(order_code: str) -> str:
  ...
  入参说明:
    order_code (str): 订单编码，通常是一串由数字和字母组成的长字符串。例如："e7164e61d584613960fd49e11ebfa68"
  ...
  返回内容示例：
    ...
    "查询结果：订单不存在，请核对订单号是否正确。注意一定要用商户订单号而非银行订单号。"
    "查询失败：失败原因"
  ...
  调用要求：
    1. 本接口的order必须是商户订单号，而非工行订单号。工行订单号为400开头的全数字,如：400073730129000742510010375136。商户订单号为字母+数字的组合，如：3e7164e61d584613960fd49e11ebfa68。如果用户提供了银行订单号，请引导用户提供商户订单号。
    2. 拿到返回字符串后，请发挥你的语义理解能力，提取出‘订单状态’和‘兑换金额’等关键信息，并以友好的 Markdown 格式呈现给用户。对于订单的状态说明必须严格使用本接口的说明，绝对禁止自己添加任何解释性的文字，以免引起用户误解。
  """
...
async def query_icbc_voucher_rules(query: str) -> str:
  ...
  用途：
  当用户咨询工行微信立减金的业务逻辑、兑换流程、兑换比例、报错码（如12003832）、有效期、补发规则、如何查询订单时调用。也包括了i豆兑换实物的兑换途径。
  支持模糊语义搜索和错误码精确匹配。
