...
async def query_voucher_order_status(order_code: str) -> str:
  ...
  返回内容示例：
    ...
    注意：
      ...
      - 已回收，立减金过期未使用被收回
      - 已失效，立减金因更换实名等原因不再可用
      - 任何其它状态（比如空)， 使用状态未知（没有查到）
      
  调用要求：
    1. 拿到返回字符串后，请发挥你的语义理解能力，提取出‘订单状态’和‘兑换金额’等关键信息，并以友好的 Markdown 格式呈现给用户。对于订单的状态和券使用状态说明必须严格使用本接口的说明，绝对禁止自己添加任何解释性的文字，以免引起用户误解。
    2. 返回结果里面的订单号(order_code字段)要显示给用户。
    3. 券的使用状态要显示给用户。
  """
