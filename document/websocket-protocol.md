...
products: 当需要给用户推荐商品时，就会有这一项。
一次可能给用户推荐多个商品。
每个商品的属性介绍：
- source：表示此商品是来自哪里，"wechat"表示是来自微信小店（目前只支持wechat）。
- appid： 如果是微信小店，那么就有此项，用来展示给用户使用。
- productID：如果是微信小店，那么就有此项，用来展示给用户使用。
- productPromotionLink：如果是微信小店，那么就有此项，用来展示给用户使用。

前端如何展示推荐商品：
在微信小程序中，用store-product组件来展示。
```
<store-product 
  appid="货源商家的AppID" 
  product-id="商品的ID" 
  product-promotion-link="你的跟佣链接"
></store-product>
```
